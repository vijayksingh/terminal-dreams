...
function FeedEvolution() {
  const { activeStep, stateEntries } = useFeed();
...
function MetricsBar() {
  const { activeStep, metrics } = useFeed();
  if (activeStep < 4) return null;

  return (
    <div className={styles.metricsBar}>
      <MetricCard label="DOM" value={metrics.domNodes} bad={metrics.domNodes > 100} good={metrics.domNodes <= 50} />
      <MetricCard label="Network" value={metrics.networkReqs} bad={metrics.networkReqs > 30} good={metrics.networkReqs <= 10} />
      <MetricCard label="Scroll FPS" value={metrics.scrollFps} bad={metrics.scrollFps < 50} good={metrics.scrollFps >= 58} />
      <MetricCard label="TTI" value={`${metrics.tti}ms`} bad={metrics.tti > 2000} good={metrics.tti <= 800} />
    </div>
  );
}
...
function PostCard({ post, showType, showLikes, showScore, showEmbedding, isPending, hasFailed, onLike }: PostCardProps) {
