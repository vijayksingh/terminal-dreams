...
import { SyncCoordinator, type LockEntry } from "../engine/sync-coordinator";
...
export function PlanningView({ activeStep }: { activeStep: number }) {
...
export function EndpointChallenge() {
...
export function TypeCard({ typeDef, revealed, onReveal }: { typeDef: TypeDef; revealed: Set<string>; onReveal: (key: string) => void }) {
...
export function TabSyncEvolution() {
...
export function TopMetricsBar() {
  const { tabs, messageCount, leaderId } = useMultiTab();
...
export function PredictionChallenge({ question, options, correctIndex, explanation }: {
  question: string;
  options: string[];
  correctIndex: number;
  explanation: string;
...
export const STEP_SCOPE_MAP: Record<number, string> = {
  4: "broadcastChannel",
  10: "storageEvents",
  11: "sharedWorker",
  7: "leaderElection",
...
export function ScopeBadge({ step, expanded, onToggle }: { step: number; expanded: boolean; onToggle: () => void }) {
...
export function StepWidget({ step }: { step: number }) {
  const { scopeEnabled } = useMultiTab();
...
export function BroadcastChannelWidget() {
...
export function MessageLog({ messages }: { messages: { id: string; type: string; from: string; to: string; payload: string; timestamp: number }[] }) {
  const noMotion = usePrefersReducedMotion();
  return (
...
export function MessageProtocolWidget() {
...
export function TabRegistryWidget() {
...
export function LeaderElectionWidget() {
...
export function StateSyncWidget() {
...
export function ConflictResolutionWidget() {
  ...
  const [result, setResult] = useState<ReturnType<typeof SyncCoordinator.resolveConflict> | null>(null);
  ...
  const handleResolve = () => {
    const r = SyncCoordinator.resolveConflict(conflictStrategy, tabAVal, tabBVal, 1, 2);
    setResult(r);
...
export function LocalStorageWidget() {
  const { markStepComplete } = useMultiTab();
  const noMotion = usePrefersReducedMotion();
...
export function SharedWorkerWidget() {
...
export function TabLifecycleWidget() {
...
export function ThrottlingWidget() {
  const { markStepComplete } = useMultiTab();
...
export function LockApiWidget() {
  ...
  const requestLock = (lockName: string, tabId: string) => {
    setLocks(prev => SyncCoordinator.requestLock(prev, lockName, tabId, () => setContentionCount(c => c + 1)));
  };

  const releaseLock = (lockName: string, tabId: string) => {
    setLocks(prev => SyncCoordinator.releaseLock(prev, lockName, tabId));
  };
